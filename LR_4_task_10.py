...
def find_similar_users(dataset, user):
    if user not in dataset:
        raise TypeError('Cannot find ' + user + ' in the dataset')
    scores = np.array([[x, pearson_score(dataset, user, x)] for x in dataset if x != user])
    scores_sorted = np.argsort(scores[:, 1])[::-1]
    top_users = scores_sorted[:3]
    return scores[top_users]

if __name__ == '__main__':
    #args = build_arg_parser().parse_args()
    user = ["Bill Duffy", "Clarissa Jackson"]
    ratings_file = 'ratings.json'
    for items in user:
        with open(ratings_file, 'r') as f:
            data = json.loads(f.read())
        print('\nUsers similar to ' + items + ':\n')
        similar_users = find_similar_users(data, items)
